...
function MainPage(): React.ReactElement {
  ...
  const offersList = useSelector((state: RootState) => state.sortedOffers); // ✅ OffersList[]
  
  // 🔹 Преобразуем OffersList[] в FullOffer[], добавляя заглушки для недостающих полей
  const offers: FullOffer[] = offersList.map((offer) => ({
    ...offer,
    description: "Default description", // 🔹 Заглушка
    bedrooms: 2, // 🔹 Заглушка
    goods: ["Wi-Fi", "Heating"], // 🔹 Заглушка
    host: {
      name: "Host",
      avatarUrl: "/img/avatar.jpg",
      isPro: false,
    },
    images: [offer.previewImage], // 🔹 Используем previewImage как изображение
    maxAdults: 2, // 🔹 Заглушка
  }));
        ...
        <div className="cities">
          <div className="cities__places-container container">
            <section className="cities__places places">
              ...
              {offersList.length > 0 ? ( // ✅ Проверяем, есть ли предложения
                <CitiesCardList offersList={offersList} />
              ) : (
                <p>No offers available in {city}.</p> // ✅ Добавлен текст при отсутствии предложений
              )}
            </section>
